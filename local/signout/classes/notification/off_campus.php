    ...
    public function __construct($id=0) {
        ...
        if ($id) {
            $record = $DB->get_record_sql(
                "SELECT u.id as student, a.id as approver, sd.hohid AS hoh, oc.type, oc.passengers, d.userid as driver,
                 ...
                 LEFT JOIN {local_signout_off_campus} d ON oc.driverid = d.id LEFT JOIN {user} a ON oc.approverid = a.id
                 LEFT JOIN {local_mxschool_student} s ON u.id = s.userid LEFT JOIN {local_mxschool_dorm} sd ON s.dormid = sd.id
