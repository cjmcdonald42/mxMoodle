...
function get_all_user_appointment_info2($userid) {
	...
	$records = $DB->get_records_sql(
		"SELECT av.id AS avid, av.userid, ht.testing_block_id, ht.attended, tb.id AS tbid, tb.testing_cycle,
				tb.start_time, tb.end_time, tb.date, tb.max_testers
		 FROM {local_mxschool_audit} av LEFT JOIN {user} u ON u.id=av.userid LEFT JOIN {local_mxschool_healthtest} ht ON u.id=ht.userid LEFT JOIN {local_mxschool_testing_block} tb ON tb.id = tb.testing_block_id
